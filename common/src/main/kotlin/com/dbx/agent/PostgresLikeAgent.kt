...
data class PostgresLikeAgentProfile(
    val driverClass: String,
    val urlTemplate: String,
) {
    fun buildUrl(params: ConnectParams): String {
        return JdbcAgentProfile(driverClass = driverClass, urlTemplate = urlTemplate).buildUrl(params)
    }
}

abstract class PostgresLikeAgent(
    open val profile: PostgresLikeAgentProfile,
) : DatabaseAgent {
    ...
    override fun listDatabases(): List<DatabaseInfo> {
        ...
        requireConnection().prepareStatement("SELECT datname FROM pg_database WHERE datistemplate = false ORDER BY datname")
            .use { stmt ->
    ...
    override fun listObjects(schema: String): List<ObjectInfo> {
        val result = listTables(schema)
            .map { ObjectInfo(name = it.name, object_type = it.table_type, schema = schema, comment = it.comment) }
            .toMutableList()
        requireConnection().prepareStatement(
    ...
    override fun getColumns(schema: String, table: String): List<ColumnInfo> {
        val conn = requireConnection()
        val primaryKeys = primaryKeys(conn, schema, table)
    ...
    private fun primaryKeys(conn: Connection, schema: String, table: String): Set<String> {
